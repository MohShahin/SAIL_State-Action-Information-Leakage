...
- [ ] **Vasopressor weaning** (`docs/PHASE_PLAN_TERMINOLOGY_REVISION.md` Phase 7) — a real methodological gap: current sepsis-RL literature struggles to learn good down-titration policies, plausibly for reasons connected to — but not identical to — this project's findings, now correctly reframed per Phase 0 as treatment-derived state information rather than leakage per se. Research question: does the same treatment-confounded-state pathway that makes the *next dose decision* recoverable from state also make it harder to learn a policy that safely *reduces* an already-running vasopressor, since the state signal that would justify weaning (falling severity) is itself entangled with the treatment being weaned? A distinct action-space/decision-boundary problem from this paper's audit, deserving its own experimental design rather than a bolt-on.
...
- [ ] **Treatment persistence as a general state-design confound** (`docs/PHASE_PLAN_TERMINOLOGY_REVISION.md` §5.5) — Phase 4's Variant F test found that two purely temporal treatment-history features (duration on vasopressor, time since last dose-tier change), with no physiology at all, predict the next vasopressor decision *more* accurately than the full physiology-plus-severity state (0.914 vs. 0.900 AUROC). That single-cohort, single-treatment-type result doesn't establish a general claim — validating it would mean replicating on a different ICU database (eICU, AmsterdamUMCdb), a different treatment type (ventilation weaning, sedation titration), a different action-space granularity, and a different history-feature choice. Assessed as empirical, not provable in the Theorem 1/2 sense (§5.5) — a natural companion to the vasopressor-weaning direction above, since both are ultimately about treatment persistence's role in sepsis-RL state design and would likely share cohort/methodology infrastructure if pursued together.
